...
export const getProducts = async () => {
  ...
  const products = data.products.edges.map(({ node: product }: any) => {
    return {
      ...
      attentionSeekersMetafield: product.attentionSeekers?.value || product.attentionSeekers2?.value || product.styleSeekers?.value || '',
      sizeFitMetafield: product.sizeFit?.value || product.sizeFit2?.value || '',
      careMetafield: product.careInstructions?.value || product.takeCare?.value || '',
