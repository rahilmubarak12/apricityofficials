...
import { Product, CartItem, Category, Collection } from '~/lib/types';
...
function mapSubCategory(tags: string[], title: string): Product['subCategory'] {
  ...
  if (lowerTags.some((t) => t.includes('set') || t.includes('linen')) || lowerTitle.includes('set') || lowerTitle.includes('linen')) return 'shirt';
...
function mapShopifyProduct(node: any, pagesMap: Record<string, { title: string; body: string }>): any {
  ...
  return {
    ...
    attentionSeekersMetafield: node.attentionSeekers?.value || node.attentionSeekers2?.value || node.styleSeekers?.value || scraped?.attentionStyleSeekers || '',
    descriptionFeaturesHtml: scraped?.descriptionFeatures || '',
    sizeFitMetafield: node.sizeFit?.value || node.sizeFit2?.value || scraped?.sizeFit || '',
    sizeChartHtml,
...
export default function Homepage() {
  ...
  // Geolocation and currency setup on mount
  useEffect(() => {
      ...
      if (!localStorage.getItem('selectedCountry')) {
        ...
        if (detectedCode) {
          if (COUNTRIES[detectedCode]) {
            ...
            const countryCode = eurozoneCodes.includes(detectedCode) ? 'EU' : 'SA';
            setSelectedCountry(countryCode);
            localStorage.setItem('selectedCountry', countryCode);
          }
        } else {
          setSelectedCountry('SA');
          localStorage.setItem('selectedCountry', 'SA');
        }
      }
    }
